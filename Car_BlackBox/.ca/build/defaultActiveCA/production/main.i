...
void main(void) {
    ...
    str_write_eeprom_24c04(0x00, "1001");

    char event[3] = "ON";
    unsigned char speed = 0, menu_pos;
    unsigned char control_flag = 0x11, key, reset_flag;
    ...
    while(1)
    {
        ...
        else if((key == 0x37 || key == 0x2F) && control_flag == 0x11 )
        {
            control_flag = 0x12;
            clear_screen();
        ...
        else if(control_flag == 0x13 && key == 0x37)
        {
            if(++delay > 200)
            {
                menu_pos = display_menu(key, reset_flag);
                switch(menu_pos)
                {
                    case 0x07:

                        clear_screen();
                        clcd_print("View Log...", (0xC0 + 2));
                        break;
                    ...
                    case 0x09:
                        clear_screen();
                        clcd_print("Clear log...", (0xC0 + 2));
                        break;

                    case 0x0A:
                        clear_screen();
                        clcd_print("Setting Time", (0xC0 + 0));
                        break;
        ...
        else if(control_flag == 0x13 && key == 0x2F)
        {
            if(++delay > 200)
            {
                control_flag = display_menu(key, reset_flag);
            }
        }
        else
        {
